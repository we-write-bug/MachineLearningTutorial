...
public class MyButton extends android.support.v7.widget.AppCompatButton {
    ...
    public MyButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MyButton, defStyle, 0);
        if (a != null) {
            //设置背景色
            ColorStateList colorList = a.getColorStateList(R.styleable.MyButton_backColor);
            if (colorList != null) {
            ...
            //记录按钮被按下时的背景色
            ColorStateList colorListPress = a.getColorStateList(R.styleable.MyButton_backColorPress);
            ...
            backGroundDrawable = a.getDrawable(R.styleable.MyButton_backGroundImage);
            if (backGroundDrawable != null) {
                setBackgroundDrawable(backGroundDrawable);
            }
            //记录按钮被按下时的背景图片
            backGroundDrawablePress = a.getDrawable(R.styleable.MyButton_backGroundImagePress);
            //设置文字的颜色
            textColor = a.getColorStateList(R.styleable.MyButton_textColor);
            if (textColor != null) {
                setTextColor(textColor);
            }
            //记录按钮被按下时文字的颜色
            textColorPress = a.getColorStateList(R.styleable.MyButton_textColorPress);
            //设置圆角或圆形等样式的背景色
            fillet = a.getBoolean(R.styleable.MyButton_fillet, false);
            if (fillet) {
                getGradientDrawable();
                if (backColor != 0) {
            ...
            float radius = a.getFloat(R.styleable.MyButton_radius, 0);
            if (fillet && radius != 0) {
                setRadius(radius);
            }
            //设置按钮形状，fillet为true时才生效
            int shape = a.getInteger(R.styleable.MyButton_shape, 0);
            if (fillet && shape != 0) {
                setShape(shape);
            }
            a.recycle();
